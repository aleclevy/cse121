...
#define DOT_DASH_THRESHOLD (UNIT_MS * 2)
#define LETTER_GAP_THRESHOLD (UNIT_MS * 2)
#define WORD_GAP_THRESHOLD (UNIT_MS * 5)
#define SENTENCE_THRESHOLD (UNIT_MS * 10)
  ESP_ERROR_CHECK(
      ...
      else
      {
        ...
        else
        {
          ...
          if (strlen(morseRaw) > sizeof(morseRaw) - 40)
            morseRaw[0] = '\0'; // repeats, don't overflow
        }
      }
      last_state = lit;
...
static void convertToEnglish_Text(const char *text, char *out)
{
  ...
  while (word != NULL)
  {
    if (!first)
      strcat(out, " ");
    first = false;
    convertToEnglish_Word(word, out);
